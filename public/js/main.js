...
window.globalMyHero = false; // Global variable for the selected player
...
window.createMyPubNub = function (currentLevel, selectedPlayer) {
  ...
  window.globalMyHero = selectedPlayer; // Get the current player and set it to the global player
    ...
    message(messageEvent) {
      ...
      if (messageEvent.channel === window.currentFireChannelName) {
        ...
        if (messageEvent.message.int === true && messageEvent.message.sendToRightPlayer === window.UniqueID) { // If you get a message and it matches with your UUID
          ...
          window.StartPlay(); // Call the game state start function in onLoad
        }
      }
...
const loadPlayerSelectionState = document.createElement('script');
loadPlayerSelectionState.src = './js/playerSelectionState.js';
document.head.appendChild(loadPlayerSelectionState);
...
window.addEventListener('load', () => {
  ...
  game.state.add('playerSelection', window.PlayerSelectionState);
  ...
  window.StartPlay = function () {
    game.state.start('play', true, false, { level: window.globalCurrentLevel }); // Start Game
  };
});
